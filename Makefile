...
bench:
	cd benchmarks && \
		go test $(LDFLAGS) -tags cleveldb,pebbledb -run=NOTEST -bench=Small . && \
		go test $(LDFLAGS) -tags cleveldb,pebbledb -run=NOTEST -bench=Medium . && \
...
fullbench:
	cd benchmarks && \
		...
		go test $(LDFLAGS) -tags cleveldb,pebbledb -run=NOTEST -bench=Small . && \
		go test $(LDFLAGS) -tags cleveldb,pebbledb -run=NOTEST -bench=Medium . && \
		go test $(LDFLAGS) -tags cleveldb,pebbledb -run=NOTEST -timeout=30m -bench=Large . && \
